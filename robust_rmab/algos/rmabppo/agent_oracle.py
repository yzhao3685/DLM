...
class AgentOracle:
    ...
    def __init__(self, data, N, S, A, B, seed, REWARD_BOUND, agent_kwargs=dict(),
        ...
        reward_from="llm", scheduler_discount=0.99999):
        ...
        if data == 'counterexample':
            self.env_fn = lambda : CounterExampleRobustEnv(N,B,seed, reward_from=self.reward_from)
    ...
    def best_response_per_cpu(self, nature_strats, nature_eq, add_to_seed, actor_critic=core.MLPActorCriticRMAB, ac_kwargs=dict(), seed=0, 
        ...
        for epoch in range(epochs):
            ...
            wlog(
                {
                    'EpActualRet': logger.epoch_dict['EpActualRet'][0],
                    'EpLambAdjRet': logger.epoch_dict['EpLambAdjRet'][0],
                    'Lamb': logger.epoch_dict['Lamb'][0],
                    'System/Time': time.time()-epoch_start_time
                },
                commit=True                
            )   
...
if __name__ == '__main__':
    ...
    parser.add_argument('--reward_from', default='llm', type=str, choices=['llm', 'human', 'base', 'default'], help='Which reward function to use in training')
    ...
    if os.getenv('USE_WANDB'):
        wandb.init(
            project="llm_reward_learning",
            entity="ezipe",
            name='train_'+args.save_string,
            config=vars(args)
        )
    mpi_fork(args.cpu, is_cannon=args.cannon)  # run parallel code with mpi
    ...
    if args.data == 'counterexample':
        ...
        env_fn = lambda : CounterExampleRobustEnv(N,B,seed,reward_from=args.reward_from)
    ...
    agent_oracle  = AgentOracle(data, N, S, A, budget, seed, reward_bound,
                             ...
                             reward_from=args.reward_from, scheduler_discount=scheduler_discount)
